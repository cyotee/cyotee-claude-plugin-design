...
1. **Check for design.yaml** - Required for repo prefix:
   ...
   if [ ! -f "design.yaml" ]; then
     echo "No design.yaml found - run /design:init first"
   fi
   ```
...
2. **Check for tasks/ directory:**
   ```bash
   if [ ! -d "tasks" ]; then
     echo "No tasks/ directory found - run /design:init first"
   fi
   ```

3. **Check file exists:**
   - If $ARGUMENTS is empty, inform user: "Usage: /design:digest <file-path>"
...
3. Use /backlog:launch to start work on Ready tasks
...
- **No tasks/ directory:** "Run /design:init first to set up the tasks directory."
- **No design.yaml:** "Run /design:init first to configure the repository."
